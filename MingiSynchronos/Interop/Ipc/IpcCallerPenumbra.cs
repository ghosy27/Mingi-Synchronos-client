...
public sealed class IpcCallerPenumbra : DisposableMediatorSubscriberBase, IIpcCaller
{
    ...
    public string? ModDirectory
    {
        ...
        private set
        {
            if (!string.Equals(_penumbraModDirectory, value, StringComparison.Ordinal))
            {
                ...
                _MingiMediator.Publish(new PenumbraDirectoryChangedMessage(_penumbraModDirectory));
            }
        }
    }
        ...
        MingiMediator MingiMediator, RedrawManager redrawManager) : base(logger, MingiMediator)
    {
        ...
        _penumbraModSettingChanged = ModSettingChanged.Subscriber(pi, (change, arg1, arg, b) =>
        {
            if (change == ModSettingChange.EnableState)
                _MingiMediator.Publish(new PenumbraModSettingChangedMessage());
        });
    ...
    public void CheckAPI()
    {
        ...
        finally
        {
            if (!penumbraAvailable && !_shownPenumbraUnavailable)
            {
                ...
                _MingiMediator.Publish(new NotificationMessage("Penumbra inactive",
                    "Your Penumbra installation is not active or out of date. Update Penumbra and/or the Enable Mods setting in Penumbra to continue to use Mingi. If you just updated Penumbra, ignore this message.",
    ...
    public async Task ConvertTextureFiles(ILogger logger, Dictionary<string, string[]> textures, IProgress<(string, int)> progress, CancellationToken token)
    {
        ...
        _MingiMediator.Publish(new HaltScanMessage(nameof(ConvertTextureFiles)));
        ...
        _MingiMediator.Publish(new ResumeScanMessage(nameof(ConvertTextureFiles)));
    ...
    public async Task SetManipulationDataAsync(ILogger logger, Guid applicationId, Guid collId, string manipulationData)
    {
        ...
        await _dalamudUtil.RunOnFrameworkThread(() =>
        {
            ...
            var retAdd = _penumbraAddTemporaryMod.Invoke("MingiChara_Meta", collId, [], manipulationData, 0);
    ...
    public async Task SetTemporaryModsAsync(ILogger logger, Guid applicationId, Guid collId, Dictionary<string, string> modPaths)
    {
        ...
        await _dalamudUtil.RunOnFrameworkThread(() =>
        {
            ...
            var retRemove = _penumbraRemoveTemporaryMod.Invoke("MingiChara_Files", collId, 0);
            logger.LogTrace("[{applicationId}] Removing temp files mod for {collId}, Success: {ret}", applicationId, collId, retRemove);
            var retAdd = _penumbraAddTemporaryMod.Invoke("MingiChara_Files", collId, modPaths, string.Empty, 0);
    ...
    private void RedrawEvent(IntPtr objectAddress, int objectTableIndex)
    {
        ...
        {
            _MingiMediator.Publish(new PenumbraRedrawMessage(objectAddress, objectTableIndex, wasRequested));
        }
    }
    ...
    private void ResourceLoaded(IntPtr ptr, string arg1, string arg2)
    {
        ...
        {
            _MingiMediator.Publish(new PenumbraResourceLoadMessage(ptr, arg1, arg2));
        }
    }
    ...
    {
        _redrawManager.Cancel();
        _MingiMediator.Publish(new PenumbraDisposedMessage());
    }
    ...
    private void PenumbraInit()
    {
        ...
        _MingiMediator.Publish(new PenumbraInitializedMessage());
