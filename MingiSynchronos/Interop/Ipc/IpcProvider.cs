...
public class IpcProvider : IHostedService, IMediatorSubscriber
{
    ...
    public MingiMediator Mediator { get; init; }

    public IpcProvider(ILogger<IpcProvider> logger, IDalamudPluginInterface pi,
        CharaDataManager charaDataManager, MingiMediator MingiMediator)
    {
        _logger = logger;
        _pi = pi;
        _charaDataManager = charaDataManager;
        Mediator = MingiMediator;
    ...
    public Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        _loadFileProvider = _pi.GetIpcProvider<string, IGameObject, bool>("MingiSynchronos.LoadMcdf");
        _loadFileProvider.RegisterFunc(LoadMcdf);
        _loadFileAsyncProvider = _pi.GetIpcProvider<string, IGameObject, Task<bool>>("MingiSynchronos.LoadMcdfAsync");
        _loadFileAsyncProvider.RegisterFunc(LoadMcdfAsync);
        _handledGameAddresses = _pi.GetIpcProvider<List<nint>>("MingiSynchronos.GetHandledAddresses");
