...
internal sealed partial class CharaDataHubUi : WindowMediatorSubscriberBase
{
    ...
    public CharaDataHubUi(ILogger<CharaDataHubUi> logger, MingiMediator mediator, PerformanceCollectorService performanceCollectorService,
        ...
        : base(logger, mediator, "Mingi Synchronos Character Data Hub###MingiSynchronosCharaDataUI", performanceCollectorService)
    {
        ...
        Mediator.Subscribe<GposeStartMessage>(this, (_) => IsOpen |= _configService.Current.OpenMingiHubOnGposeStart);
    ...
    {
        _uiSharedService.BigText("Mingi Character Data File Export");

        DrawHelpFoldout("This feature allows you to pack your character into a MCDF file and manually send it to other people. MCDF files can officially only be imported during GPose through Mingi. " +
        ...
        if (_readExport)
        {
            ...
            if (_uiSharedService.IconTextButton(FontAwesomeIcon.Save, "Export Character as MCDF"))
            {
                ...
                _uiSharedService.FileDialogManager.SaveFileDialog("Export Character to file", ".mcdf", defaultFileName, ".mcdf", (success, path) =>
                {
                    ...
                    _charaDataManager.SaveMingiCharaFile(_exportDescription, path);
    ...
    private void DrawSettings()
    {
        ...
        ImGuiHelpers.ScaledDummy(5);
        bool openInGpose = _configService.Current.OpenMingiHubOnGposeStart;
