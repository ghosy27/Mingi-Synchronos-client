...
public class TopTabMenu
{
    ...
    public TopTabMenu(MingiMediator MingiMediator, ApiController apiController, PairManager pairManager, UiSharedService uiSharedService)
    {
    ...
    private void DrawSyncshellMenu(float availableWidth, float spacingX)
    {
            ...
            .Count(g => string.Equals(g.OwnerUID, _apiController.UID, StringComparison.Ordinal)) >= _apiController.ServerInfo.MaxGroupsCreatedByUser))
        {
            ...
            {
                _MingiMediator.Publish(new UiToggleMessage(typeof(CreateSyncshellUI)));
            }
            ImGui.SameLine();
        }
        ...
        using (ImRaii.Disabled(_pairManager.GroupPairs.Select(k => k.Key).Distinct().Count() >= _apiController.ServerInfo.MaxGroupsJoinedByUser))
        {
            ...
            {
                _MingiMediator.Publish(new UiToggleMessage(typeof(JoinSyncshellUI)));
            }
        }
    }
    ...
    {
        var buttonX = (availableWidth - spacingX) / 2f;
        if (_uiSharedService.IconTextButton(FontAwesomeIcon.UserCircle, "Edit Mingi Profile", buttonX))
        {
            _MingiMediator.Publish(new UiToggleMessage(typeof(EditProfileUi)));
        }
        UiSharedService.AttachToolTip("Edit your Mingi Profile");
        ImGui.SameLine();
        if (_uiSharedService.IconTextButton(FontAwesomeIcon.PersonCircleQuestion, "Chara Data Analysis", buttonX))
        {
            _MingiMediator.Publish(new UiToggleMessage(typeof(DataAnalysisUi)));
        }
        ...
        {
            _MingiMediator.Publish(new UiToggleMessage(typeof(CharaDataHubUi)));
        }
    }
