...
using MingiSynchronos.Localization;
...
public partial class UiSharedService : DisposableMediatorSubscriberBase
{
    ...
    private const string _notesEnd = "##Mingi_SYNCHRONOS_USER_NOTES_END##";
    private const string _notesStart = "##Mingi_SYNCHRONOS_USER_NOTES_START##";
    ...
    public UiSharedService(ILogger<UiSharedService> logger, IpcManager ipcManager, ApiController apiController,
        ...
        MingiConfigService configService, DalamudUtilService dalamudUtil, IDalamudPluginInterface pluginInterface,
        ITextureProvider textureProvider,
        Dalamud.Localization localization,
        ServerConfigurationManager serverManager, TokenProvider tokenProvider, MingiMediator mediator) : base(logger, mediator)
    {
    ...
    {
        ColorTextWrapped("Note: The storage folder should be somewhere close to root (i.e. C:\\MingiStorage) in a new empty folder. DO NOT point this to your game folder. DO NOT point this to your Penumbra folder.", ImGuiColors.DalamudYellow);
        ...
        ImGui.SameLine();
        using (ImRaii.Disabled(_cacheMonitor.MingiWatcher != null))
        {
            if (IconButton(FontAwesomeIcon.Folder))
            {
                FileDialogManager.OpenFolderDialog("Pick Mingi Synchronos Storage Folder", (success, path) =>
                {
                    if (!success) return;
                    ...
                    {
                        _cacheDirectoryHasOtherFilesThanCache = true;
                        Logger.LogWarning("Found folders in {path} not belonging to Mingi: {dirs}", path, string.Join(", ", dirs));
                    }
                        ...
                        && _cacheDirectoryIsValidPath)
                    {
                        ...
                        _configService.Save();
                        _cacheMonitor.StartMingiWatcher(path);
                        _cacheMonitor.InvokeScan();
                    }
                }, _dalamudUtil.IsWine ? @"Z:\" : @"C:\");
            }
        }
        if (_cacheMonitor.MingiWatcher != null)
        {
        ...
        {
            ColorTextWrapped("Your selected directory has files or directories inside that are not Mingi related. Use an empty directory or a previous Mingi storage directory only.", ImGuiColors.DalamudRed);
        }
        ...
        DrawHelpText("The storage is automatically governed by Mingi. It will clear itself automatically once it reaches the set capacity by removing the oldest unused files. You typically do not need to clear it yourself.");
    }
    ...
    public bool DrawOtherPluginState()
    {
        ...
        {
            ImGui.TextColored(ImGuiColors.DalamudRed, "You need to install both Penumbra and Glamourer and keep them up to date to use Mingi Synchronos.");
            return false;
        }
