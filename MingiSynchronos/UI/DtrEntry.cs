...
public sealed class DtrEntry : IDisposable, IHostedService
{
    ...
    private readonly ConfigurationServiceBase<MingiConfig> _configService;
    ...
    public DtrEntry(ILogger<DtrEntry> logger, IDtrBar dtrBar, ConfigurationServiceBase<MingiConfig> configService, MingiMediator MingiMediator, PairManager pairManager, ApiController apiController)
    {
        _logger = logger;
        _dtrBar = dtrBar;
    ...
    private IDtrBarEntry CreateEntry()
    {
        ...
        var entry = _dtrBar.Get("Mingi Synchronos");
        entry.OnClick = _ => _MingiMediator.Publish(new UiToggleMessage(typeof(CompactUi)));
    ...
    private void Update()
    {
        ...
        if (_apiController.IsConnected)
        {
            ...
            if (pairCount > 0)
            {
                ...
                tooltip = $"Mingi Synchronos: Connected{Environment.NewLine}----------{Environment.NewLine}{string.Join(Environment.NewLine, visiblePairs)}";
                colors = _configService.Current.DtrColorsPairsInRange;
            }
            else
            {
                tooltip = "Mingi Synchronos: Connected";
        ...
        {
            text = "\uE044 \uE04C";
            tooltip = "Mingi Synchronos: Not Connected";
