...
public partial class ApiController
{
    ...
    {
        if (!IsConnected) return;
        await _mingiHub!.SendAsync(nameof(UserAddPair), user).ConfigureAwait(false);
    }

    public async Task UserDelete()
    {
        CheckConnection();
        await _mingiHub!.SendAsync(nameof(UserDelete)).ConfigureAwait(false);
    ...
    {
        return await _mingiHub!.InvokeAsync<List<OnlineUserIdentDto>>(nameof(UserGetOnlinePairs), censusDataDto).ConfigureAwait(false);
    }

    public async Task<List<UserFullPairDto>> UserGetPairedClients()
    {
        return await _mingiHub!.InvokeAsync<List<UserFullPairDto>>(nameof(UserGetPairedClients)).ConfigureAwait(false);
    }
    ...
    public async Task<UserProfileDto> UserGetProfile(UserDto dto)
    {
        ...
        return await _mingiHub!.InvokeAsync<UserProfileDto>(nameof(UserGetProfile), dto).ConfigureAwait(false);
    }
    ...
    public async Task UserPushData(UserCharaDataMessageDto dto)
    {
        try
        {
            await _mingiHub!.InvokeAsync(nameof(UserPushData), dto).ConfigureAwait(false);
        }
        catch (Exception ex)
        {
    ...
    public async Task SetBulkPermissions(BulkPermissionsDto dto)
    {
        ...
        try
        {
            await _mingiHub!.InvokeAsync(nameof(SetBulkPermissions), dto).ConfigureAwait(false);
        }
        catch (Exception ex)
        {
    ...
    {
        if (!IsConnected) return;
        await _mingiHub!.SendAsync(nameof(UserRemovePair), userDto).ConfigureAwait(false);
    }
    ...
    {
        if (!IsConnected) return;
        await _mingiHub!.InvokeAsync(nameof(UserSetProfile), userDescription).ConfigureAwait(false);
    }

    public async Task UserUpdateDefaultPermissions(DefaultPermissionsDto defaultPermissionsDto)
    {
        CheckConnection();
        await _mingiHub!.InvokeAsync(nameof(UserUpdateDefaultPermissions), defaultPermissionsDto).ConfigureAwait(false);
    }
