...
using MingiSynchronos.API.Data.Extensions;
using MingiSynchronos.API.Dto;
using MingiSynchronos.API.Dto.User;
using MingiSynchronos.API.SignalR;
...
public sealed partial class ApiController : DisposableMediatorSubscriberBase, IMingiHubClient
{
    ...
    public const string MainServiceUri = "https://fxiv.ghosy.site:6000";
        ...
        TokenProvider tokenProvider, MingiConfigService mingiConfigService) : base(logger, mediator)
    {
        ...
        Mediator.Subscribe<HubClosedMessage>(this, (msg) => MingiHubOnClosed(msg.Exception));
        Mediator.Subscribe<HubReconnectedMessage>(this, (msg) => _ = MingiHubOnReconnectedAsync());
        Mediator.Subscribe<HubReconnectingMessage>(this, (msg) => MingiHubOnReconnecting(msg.Exception));
    ...
    public async Task CreateConnectionsAsync()
    {
        ...
        if (!_serverManager.CurrentServer.UseOAuth2)
        {
            ...
            if (multi)
            {
                ...
                _connectionDto = null;
                Mediator.Publish(new NotificationMessage("Multiple Identical Characters detected", "Your Service configuration has multiple characters with the same name and world set up. Delete the duplicates in the character management to be able to connect to Mingi.",
        ...
        while (ServerState is not ServerState.Connected && !token.IsCancellationRequested)
        {
            ...
            try
            {
                ...
                catch (MingiAuthFailureException ex)
                {
                ...
                if (_connectionDto.ServerVersion != IMingiHub.ApiVersion)
                {
                ...
                if (_dalamudUtil.IsLodEnabled && !_naggedAboutLod)
                {
                    ...
                    if (!_mingiConfigService.Current.DebugStopWhining)
                    {
                        Mediator.Publish(new NotificationMessage("Model LOD is enabled",
                            ...
                            "random crashes when loading in or rendering modded pairs. Disabling LOD has a very low performance impact. Disable LOD while using Mingi: " +
    ...
    private void MingiHubOnClosed(Exception? arg)
    {
        _healthCheckTokenSource?.Cancel();
    ...
    private async Task MingiHubOnReconnectedAsync()
    {
        ServerState = ServerState.Reconnecting;
        try
        {
    ...
    private void MingiHubOnReconnecting(Exception? arg)
    {
