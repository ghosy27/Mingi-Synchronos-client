...
using NotificationType = MingiSynchronos.MingiConfiguration.Models.NotificationType;
...
public class NotificationService : DisposableMediatorSubscriberBase, IHostedService
{
    ...
    private readonly MingiConfigService _configurationService;

    public NotificationService(ILogger<NotificationService> logger, MingiMediator mediator,
        DalamudUtilService dalamudUtilService,
        INotificationManager notificationManager,
        IChatGui chatGui, MingiConfigService configurationService) : base(logger, mediator)
    {
    ...
    {
        SeStringBuilder se = new SeStringBuilder().AddText("[Mingi Synchronos] Error: " + message);
        _chatGui.PrintError(se.BuiltString);
    }

    private void PrintInfoChat(string? message)
    {
        SeStringBuilder se = new SeStringBuilder().AddText("[Mingi Synchronos] Info: ").AddItalics(message ?? string.Empty);
        _chatGui.Print(se.BuiltString);
    }

    private void PrintWarnChat(string? message)
    {
        SeStringBuilder se = new SeStringBuilder().AddText("[Mingi Synchronos] ").AddUiForeground("Warning: " + (message ?? string.Empty), 31).AddUiForegroundOff();
        _chatGui.Print(se.BuiltString);
    }
