...
public sealed class CharaDataFileHandler : IDisposable
{
    ...
    private readonly MingiCharaFileDataFactory _MingiCharaFileDataFactory;
            ...
            DalamudUtilService dalamudUtilService, GameObjectHandlerFactory gameObjectHandlerFactory, PlayerDataFactory playerDataFactory)
    {
        ...
        _MingiCharaFileDataFactory = new(fileCacheManager);
    }
    ...
    public Task<(MingiCharaFileHeader loadedCharaFile, long expectedLength)> LoadCharaFileHeader(string filePath)
    {
        try
        {
            ...
            var loadedCharaFile = MingiCharaFileHeader.FromBinaryReader(filePath, reader);

            _logger.LogInformation("Read Mingi Chara File");
    ...
    public Dictionary<string, string> McdfExtractFiles(MingiCharaFileHeader? charaFileHeader, long expectedLength, List<string> extractedFiles)
    {
        ...
        using var reader = new BinaryReader(lz4Stream);
        MingiCharaFileHeader.AdvanceReaderToData(reader);

        long totalRead = 0;
        Dictionary<string, string> gamePathToFilePath = new(StringComparer.Ordinal);
        foreach (var fileData in charaFileHeader.CharaFileData.Files)
        {
            var fileName = Path.Combine(_fileCacheManager.CacheFolder, "Mingi_" + _globalFileCounter++ + ".tmp");
    ...
    internal async Task SaveCharaFileAsync(string description, string filePath)
    {
        ...
        try
        {
            ...
            var MingiCharaFileData = _MingiCharaFileDataFactory.Create(description, data);
            MingiCharaFileHeader output = new(MingiCharaFileHeader.CurrentVersion, MingiCharaFileData);
        ...
        {
            _logger.LogError(ex, "Failure Saving Mingi Chara File, deleting output");
